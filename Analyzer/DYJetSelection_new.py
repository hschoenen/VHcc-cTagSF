...
#     is_W_mass_CR[0]     = 0
    ...
    # ??!! ====== NEW ===== !!??
    # write modified branches instead of running multiple times over the (almost) same data
...
#    met_signif[0]          = entry.MET_significance
    ...
    for i in range(0, len(jetPt)):
        ...
        # ToDo: keep event if any of the jetpt(JEC) is >20
...
#        if jet.DeltaR(Z_cand) < 0.5: continue
        ...
        # not anymore in ULv2 Jet_btagCMVA.push_back(entry.Jet_btagCMVA[i])
        jet_btagDeepB.push_back(entry.Jet_btagDeepB[i])
        # not anymore in ULv2 jet_btagDeepC.push_back(entry.Jet_btagDeepC[i])
